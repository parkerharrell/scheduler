...
//			$hoursOptions[] = array( $i, sprintf('%02d', $i) );
		// remove minutes
			$option = str_replace( ':00', '', $option );
			$hoursOptions[] = array( $i, $option );
			$t->modify( "+1 hours" );
			}
	...
	case 'submit':
		...
		$submittedValue_Hours = $req->getParam( $id_Qty_Hours );
		$submittedValue_Minutes = $req->getParam( $id_Qty_Minutes );
